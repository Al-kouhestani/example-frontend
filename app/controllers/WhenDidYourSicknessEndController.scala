...
class WhenDidYourSicknessEndController @Inject()(
                                        ...
                                        formProvider: WhenDidYourSicknessEndFormProvider,
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form(start).bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(WhenDidYourSicknessEndPage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(WhenDidYourSicknessEndPage, mode, updatedAnswers))
      )
  }
}
