...
class WhatIsYourDOBControllerSpec extends SpecBase with MockitoSugar {
  ...
  def getRequest(): FakeRequest[AnyContentAsEmpty.type] =
    FakeRequest(GET, whatIsYourDOBRoute)

  def postRequest(): FakeRequest[AnyContentAsFormUrlEncoded] =
    FakeRequest(POST, whatIsYourDOBRoute)
  ...
  "WhatIsYourDOB Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[WhatIsYourDOBView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(WhatIsYourDOBPage, validAnswer).success.value
